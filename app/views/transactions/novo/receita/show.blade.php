...
{{ Form::model( $transaction, [ 'method' => 'PATCH', 'route' =>[ 'financeiro.update', $transaction->id ], 'class'=>'collapse colapsed in', 'id'=>"transaction_show" ] ) }}
	...
	<div class="panel panel-primary">
		...
		<div class="panel-heading">	
			...
			<h3 class="panel-title">DETALHAMENTO DA RECEITA</h3>
		</div>

		@if ( $transaction->isOverdue() )
			<div class="list-group-item alert-warning text-center">
				<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				<strong><i class="fa fa-warning"></i></strong> Lançamento atrasado <strong class="timeago" title="{{ $transaction->date }}">{{ $transaction->date }}</strong>
			</div>	
		@endif
			...
			<table class="table table-hover">
				<tbody>
					...
					@foreach ( $recurring_transactions as $recurring_transaction )
						...
						@include('transactions.novo.receita.list-item', array('transaction' => $recurring_transaction))
