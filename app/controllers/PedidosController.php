...
class PedidosController extends \BaseController {
	...
	public function index()
	{
      ...
      //$pedidosBKP = Pedido::all();
      // $pedidos    = new CreatePedidosTable;    
      // $pedidos->down();
      // //sleep(3);
      // $pedidos->up();
      // // foreach ($pedidosBKP as $pedido) {
      // //    Pedido::create($pedido->toArray());
      // // }
      // echo "feito";
      // exit;

      // $mandrillKey = Config::get('services.mandrill.secret') ;
      
      // new Mandrill( $mandrillKey );
      // exit;
      ...
      if($id){
         ...
         if($pedido){
            ...
            if(!$pedido->fornecedor){
                ...
                $alert[] = [   'class'   => 'alert-warning',
                    'message'   =>'<i class="fa fa-3x fa-warning pull-left"></i> O Fornecedor deste pedido foi excluído!<br/>Por favor informe outro e salve novamente.' ];   
                Session::flash('alerts', $alert);
            ...
            for ( $i = 0; $i < count( current($itens) ); $i++ ) { // Loop no primeiro item pra pegar a quantidade de linhas
